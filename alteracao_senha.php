...
<body class="bg-[#f8f6fb] text-gray-800 fixed inset-0 p-4 lg:p-6 flex overflow-hiddentransition-colors duration-500 ">
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden relative">
            <header class="h-28 px-8 lg:px-10 flex justify-between items-center border-b border-purple-100/50 bg-white/40 backdrop-blur-md shrink-0 transition-all duration-300">
                <div class="flex items-center gap-4 relative z-10">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-100 to-amber-200 text-orange-600 flex items-center justify-center shadow-inner shrink-0 hidden sm:flex">
                        <i class="fa-solid fa-user-secret text-2xl drop-shadow-sm"></i>
                    </div>

                    <div>
                        <h2 class="text-2xl lg:text-3xl font-extrabold text-gray-800 tracking-tight">Alteração de Senha</h2>
                        <p class="text-sm lg:text-lg text-gray-500 font-medium mt-1">Gerenciamento de credencial de acesso</p>
                    </div>
                </div>
                <a href="inicio.php" class="px-5 py-2.5 bg-white border border-purple-100 text-gray-600 hover:text-purple-600 hover:bg-purple-50 rounded-xl font-bold shadow-sm transition-all flex items-center gap-2 shrink-0 relative z-10">
                    <i class="fa-solid fa-arrow-left"></i> <span class="hidden sm:inline">Voltar</span>
                </a>
            </header>

            <div class="flex-1 p-6 lg:p-8 flex items-center justify-center overflow-hidden transition-all duration-300 relative">
                
                <div class="w-full max-w-xl bg-white/80 backdrop-blur-xl border border-white/50 rounded-[2rem] shadow-xl p-8 relative z-10">
                    
                    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-5 mb-6 text-center sm:text-left border-b border-orange-100/50 pb-6 relative z-10">
                        
                        <div class="flex-1">
                            <h3 class="text-2xl font-extrabold text-gray-800 tracking-tight relative z-10">Alteração de Senha</h3>
                            <p class="text-[13px] text-gray-500 font-medium mt-1.5 leading-relaxed relative z-10">Crie uma nova senha forte e exclusiva. Lembre-se de não compartilhar seus dados de acesso com terceiros para manter a integridade do Portal.</p>
                        </div>
                    </div>

                    <form id="formAlterarSenha" class="space-y-5 relative z-10">
                        <div class="relative z-10">
                            <label class="block text-[14px] font-bold text-gray-700 mb-1.5 relative z-10">Senha Atual</label>
                            <div class="relative z-10">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                                    <i class="fa-solid fa-lock text-gray-400 relative z-10"></i>
                                </div>
                                <input type="password" id="senha_atual" name="senha_atual" placeholder="Digite sua senha atual" class="w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-100 bg-white/50 text-gray-800 focus:ring-0 focus:border-orange-400 outline-none transition-all placeholder-gray-400 font-medium relative z-10">
                            </div>
                        </div>

                        <div class="pt-3 border-t border-gray-100 relative z-10">
                            <label class="block text-[14px] font-bold text-gray-700 mb-1.5 relative z-10">Nova Senha</label>
                            <div class="relative z-10">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                                    <i class="fa-solid fa-key text-gray-400 relative z-10"></i>
                                </div>
                                <input type="password" id="nova_senha" name="nova_senha" placeholder="Crie uma nova senha" class="w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-100 bg-white/50 text-gray-800 focus:ring-0 focus:border-orange-400 outline-none transition-all placeholder-gray-400 font-medium relative z-10">
                            </div>
                            <div class="bg-gray-50 rounded-xl p-2.5 mt-2.5 border border-gray-100/80 relative z-10">
                                <p id="senha_dica" class="text-[12px] text-gray-500 font-medium transition-colors flex items-center relative z-10">
                                    <i class="fa-solid fa-circle-info mr-2 relative z-10"></i> Mín. 8 caracteres, 1 maiúscula, 1 número e 1 especial (@, -, etc).
                                </p>
                            </div>
                        </div>

                        <div class="relative z-10">
                            <label class="block text-[14px] font-bold text-gray-700 mb-1.5 relative z-10">Confirme a Nova Senha</label>
                            <div class="relative z-10">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                                    <i class="fa-solid fa-check-double text-gray-400 relative z-10"></i>
                                </div>
                                <input type="password" id="confirma_senha" name="confirma_senha" placeholder="Repita a nova senha criada" class="w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-100 bg-white/50 text-gray-800 focus:ring-0 focus:border-orange-400 outline-none transition-all placeholder-gray-400 font-medium relative z-10">
                            </div>
                        </div>

                        <div class="pt-5 flex justify-end relative z-10">
                            <button id="btnSalvarNovaSenha" type="button" class="w-full sm:w-auto px-8 py-3 rounded-xl font-extrabold text-white bg-gradient-to-r from-amber-400 to-orange-500 hover:from-amber-500 hover:to-orange-600 shadow-lg shadow-orange-500/30 hover:scale-[1.02] transition-all text-[14px] relative z-10">
                                <i class="fa-solid fa-floppy-disk mr-2 relative z-10"></i> Salvar Nova Senha
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        inputNovaSenha.addEventListener('input', function() {
            ...
            if (senha.length === 0) {
                dicaSenha.className = "text-[12px] text-gray-500 font-medium transition-colors flex items-center relative z-10";
                dicaSenha.innerHTML = '<i class="fa-solid fa-circle-info mr-2 relative z-10"></i> Mín. 8 caracteres, 1 maiúscula, 1 número e 1 especial (@, -, etc).';
            } else if (regexForte.test(senha)) {
                dicaSenha.className = "text-[12px] text-green-600 font-bold transition-colors flex items-center relative z-10";
                dicaSenha.innerHTML = '<i class="fa-solid fa-check-circle mr-2 relative z-10"></i> Senha forte e segura!';
            } else {
                dicaSenha.className = "text-[12px] text-red-500 font-bold transition-colors flex items-center relative z-10";
                dicaSenha.innerHTML = '<i class="fa-solid fa-triangle-exclamation mr-2 relative z-10"></i> Senha fraca. Siga as regras acima.';
            }
        });
        ...
        btnSalvarSenha.onclick = async function(e) {
            ...
            const regexForte = /^(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/;
            if (!regexForte.test(nova)) { mostrarNotificacao("A nova senha não atende aos requisitos de segurança.", false); return; }
            ...
            try {
                ...
                try {
                    ...
                    if (dados.sucesso) { formSenha.reset(); mostrarNotificacao(dados.mensagem, true); setTimeout(() => { window.location.href = 'inicio.php'; }, 2000); } 
                    else { mostrarNotificacao(dados.mensagem, false); btnSalvarSenha.disabled = false; btnSalvarSenha.innerHTML = '<i class="fa-solid fa-floppy-disk mr-2"></i> Salvar Nova Senha'; }
                } catch (e) { mostrarNotificacao("Erro interno no servidor.", false); btnSalvarSenha.disabled = false; btnSalvarSenha.innerHTML = '<i class="fa-solid fa-floppy-disk mr-2"></i> Salvar Nova Senha'; }
            } catch (erro) { mostrarNotificacao("Falha de comunicação com a rede.", false); btnSalvarSenha.disabled = false; btnSalvarSenha.innerHTML = '<i class="fa-solid fa-floppy-disk mr-2"></i> Salvar Nova Senha'; }
        };
    </script>
</body>
</html>